...
void main(){
	...
	// Material properties
        vec3 MaterialDiffuseColor = HSV2RGB(vec3(valence_field, 1., 1.));//texture( myTextureSampler, UV ).rgb;
	...
	color = 
        ...
        //color = 0.25*color + 0.75*vec3(valence_field, 1.-valence_field,0.);
        //color = 0.25*color + 0.75*HSV2RGB(vec3(valence_field, 1., 1.));
}
